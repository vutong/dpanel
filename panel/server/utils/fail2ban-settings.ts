...
export function validateFail2banSettings(input: unknown): { ok: true; settings: Fail2banSettings } | { ok: false; error: string } {
  ...
  for (const [name, cfg] of Object.entries(settings.jails)) {
    ...
    if (cfg.bantime < 60 || cfg.bantime > 86400) {
      return { ok: false, error: `${name}: bantime must be 60–86400 seconds` }
    }
  }
