  ...
  <div v-if="open" class="stream-backdrop" @click.self="onBackdropClick">
    <div class="stream-modal" role="dialog" :aria-labelledby="titleId">
      <header class="stream-header">
        <div>
          <h2 :id="titleId">Update Dpanel</h2>
          <p class="stream-sub">sudo dpanel update</p>
        </div>
      ...
      <p v-if="phase === 'error'" class="stream-status-msg stream-status-msg--err">
        Update failed — review the log below, then close when done.
      </p>
      <p v-if="phase === 'ok'" class="stream-status-msg stream-status-msg--ok">
        Update finished. The panel may have restarted briefly — refresh if something looks stale.
      </p>
...
const titleId = `dpanel-update-${Math.random().toString(36).slice(2, 9)}`
const logText = ref('')
...
async function fetchStatus() {
  ...
  await fetchLogChunk()
  const msg =
    s.message || (s.status === 'ok' ? 'Update complete' : 'Update failed')
  emit('done', { ok: s.status === 'ok', message: msg })
}
