#!/usr/bin/env bash
# Usage: site-unsuspend.sh <domain>
# Clear suspendedAt, restore nginx, start Node container if needed.
set -euo pipefail
...
for s in sites:
    if s.get("domain") == domain:
        if (s.get("pendingDeleteAt") or "").strip():
            print("PENDING")
            sys.exit(2)
        if not (s.get("suspendedAt") or "").strip():
            print("NOT_SUSPENDED")
            sys.exit(3)
...
PY
  ...
  code=$?
  [[ "${code}" -eq 2 ]] && die "Site is pending delete — restore instead"
  [[ "${code}" -eq 3 ]] && die "Site is not suspended"
  die "Site not found in sites.json"
}

[[ "${RUNTIME}" != "PENDING" ]] || die "Site is pending delete — restore instead"
[[ "${RUNTIME}" != "NOT_SUSPENDED" ]] || die "Site is not suspended"

SLUG="$(site_slug "${DOMAIN}")"
log "Unsuspending website: ${DOMAIN} (runtime=${RUNTIME})"
...
nginx_test_stack 1 || die "nginx -t failed after unsuspend"
nginx_reload_stack 2>/dev/null || true

printf '{"ok":true,"domain":"%s","unsuspended":true,"runtime":"%s"}\n' "${DOMAIN}" "${RUNTIME}"
log "Done — ${DOMAIN} unsuspended"
